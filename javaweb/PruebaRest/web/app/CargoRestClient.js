...
// Model for Cargo entity
    models.Cargo = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/PruebaRest/webresources/entidades.cargo/",
        idAttribute: 'idCargo',
        defaults: {
            estado: "",
            descripcionCargo: ""
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('idCargo');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/PruebaRest/webresources/entidades.cargo/';
            }
    ...
    // Collection class for Cargo entities
    models.CargoCollection = Backbone.Collection.extend({
        model: models.Cargo,
        url: "http://localhost:8080/PruebaRest/webresources/entidades.cargo/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.CargoCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-cargo-list-itemis template identifier for item
                        templateName: '#tpl-cargo-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.cargo = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.cargo,
                    // tpl-cargo-details is template identifier for chosen model element
                    templateName: '#tpl-cargo-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Cargo();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-cargo-details is a template identifier for chosen model element
                templateName: '#tpl-cargo-details',
